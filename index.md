## Selected Projects in data science, Machine Learning and Data analysis.

### Interactive Visualization of Traffic Accidents in Barranquilla

This project focuses on geocoding and analyzing traffic accidents in Barranquilla. Initially, addresses are geocoded to obtain latitudes and longitudes, and data cleaning is performed. Subsequently, an interactive dashboard is created using Tableau to visualize and explore the accidents. The outcome is a tool that provides valuable information about road safety in Barranquilla.

<img src="https://github.com/Pking31/Pking31.github.io/blob/master/images/DASHBOARD.png?raw=true">
<img src="https://github.com/Pking31/Pking31.github.io/blob/master/images/accideporfecha.png?raw=true">

[![](https://img.shields.io/badge/Python-white?logo=Python)](#) [![](https://img.shields.io/badge/Jupyter-white?logo=Jupyter)](#) [![](https://img.shields.io/badge/pandas-white?logo=data:image/png;base64,)](#) [![](https://img.shields.io/badge/Tableu-white?logo=tableau)](#)


[View code on Github](https://github.com/Pking31/Accidents_Bquilla) 

---

### PM2.5 Concentration Prediction using Machine Learning

The "PM2.5 Concentration Prediction using Machine Learning" project focuses on using meteorological data and other environmental features to predict PM2.5 particle concentration in the air. The process involves data exploration and cleaning, feature engineering, and building a regression model using XGBoost. The model is trained and fine-tuned, achieving significantly improved accuracy through parameter optimization. The project can be found in the GitHub repository with the name mentioned above.

<img src="https://github.com/Pking31/Pking31.github.io/blob/master/images/pm2.png?raw=true">

[![](https://img.shields.io/badge/Python-white?logo=Python)](#) [![](https://img.shields.io/badge/Jupyter-white?logo=Jupyter)](#) [![](https://img.shields.io/badge/pandas-white?logo=data:image/png;base64,)](#) [![](https://img.shields.io/badge/sklearn-white?logo=scikit-learn)](#)  

[View code on Github](https://github.com/Pking31/PollutionAir) 

---
### Predicting Student Scores ICFES: Machine Learning Model Utilizing Socioeconomic Factors.

This project aims to develop a machine learning model to predict a student's overall score using socioeconomic conditions as features. It involves extracting and cleaning data, performing exploratory data analysis, and implementing a linear regression model for prediction and evaluation.


<img src="https://github.com/Pking31/Pking31.github.io/blob/master/images/eda.png?raw=true" alt="Texto alternativo" width="500" height="400">

[Data source: Datos abiertos de Colombia](https://www.datos.gov.co/Educaci-n/Saber-11-2019-2/ynam-yc42)

[![](https://img.shields.io/badge/Python-white?logo=Python)](#) [![](https://img.shields.io/badge/Jupyter-white?logo=Jupyter)](#) [![](https://img.shields.io/badge/pandas-white?logo=data:image/png;base64,)](#) [![](https://img.shields.io/badge/sklearn-white?logo=scikit-learn)](#)  

[View code on Github](https://github.com/Pking31/Icfes_2019)  

---
### ColombiaTrans-Insights

Dispatches, passengers, and beyond: Comprehensive analysis of public transportation in Colombia. Understand travel patterns with processed data and generated tables.


<img src="https://github.com/Pking31/Pking31.github.io/blob/0f296b28b13ed814c2afdd1dce753c3cf1e599a4/images/dash.png?raw=true">

[Data source: Datos abiertos de Colombia]( https://www.datos.gov.co/Transporte/Operaci-n-de-pasajeros-y-despacho-de-veh-culos-en-/eh75-8ah6)

[![](https://img.shields.io/badge/Python-white?logo=Python)](#) [![](https://img.shields.io/badge/Jupyter-white?logo=Jupyter)](#) [![](https://img.shields.io/badge/pandas-white?logo=data:image/png;base64,)](#) [![](https://img.shields.io/badge/sklearn-white?logo=scikit-learn)](#)  

[View code on Github](https://github.com/Pking31/ColombiaTrans-Insights)


---
### Dashboard: Admissions in Higher Education in Colombia
In the following dashboard, you can find information about enrollments, admissions, and new students in universities (among others) in Colombia between 2017 and 2020.
Some steps were carried out as a result of the objective.

#### Analysis and understanding of data:
I conducted a thorough analysis of the available data to understand its structure, quality and characteristics. I identified the deficiencies and areas for improvement, as well as the specific requirements necessary to develop an effective dashboard.

#### Data transformation and cleansing:
Implemented data transformation and cleansing processes to address identified deficiencies. This included data normalization, outlier correction, and any other transformations needed to improve the quality and utility of the data.

<img src="https://github.com/Pking31/Pking31.github.io/blob/master/images/primerbi.png?raw=true" alt="Texto alternativo" width="500" height="490">

[Source: Ministerio de Educación Nacional](https://snies.mineducacion.gov.co/portal/ESTADISTICAS/Bases-consolidadas/)

[View code on Github](https://github.com/Pking31/PowerBI_project)  

---
### Analysis and recommender system of Movies:

The project involves developing a movie recommendation system using machine learning. The data is analyzed and transformed, including cleaning, unnesting nested fields, handling null values, and creating new columns. An exploratory data analysis is performed using Power BI, and an API is developed using FastAPI. The recommender system is based on genre similarity and returns a list of the 5 most similar movies. Tools used include FastAPI, uvicorn, Python, pandas, NumPy, scikit-learn, and Power BI.  

<img src="https://github.com/Pking31/Movies/blob/a0a72828e7c02c06ae2e08453eedc6d5f17cd242/imag/BI.JPG?raw=true"  alt="Figure 1: Dashborad on Power BI"> 

[View code on Github](https://github.com/Pking31/Movies)  

[![](https://img.shields.io/badge/Python-white?logo=Python)](#) [![](https://img.shields.io/badge/Jupyter-white?logo=Jupyter)](#) [![](https://img.shields.io/badge/pandas-white?logo=data:image/png;base64,)](#) [![](https://img.shields.io/badge/sklearn-white?logo=scikit-learn)](#)  







